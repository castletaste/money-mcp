...
export function registerTagTools(server: McpServer, db: Database) {
    ...
    try {
      ...
      return {
        ...
        content: [
          {
            type: "text" as const,
            text: `list_tags failed: ${err instanceof Error ? err.message : String(err)}`,
          },
        ],
      };
    }
  ...
  server.tool(
    ...
    {
      name: z.string().min(1).max(255).describe("Tag name (must be unique)"),
    },
    async ({ name }) => {
      try {
        const id = uuidv7();
        ...
        return {
          ...
          content: [
            {
              type: "text" as const,
              text: `create_tag failed: ${err instanceof Error ? err.message : String(err)}`,
            },
          ],
        };
      }
      ...
      try {
        ...
        return {
          ...
          content: [
            {
              type: "text" as const,
              text: `delete_tag failed: ${err instanceof Error ? err.message : String(err)}`,
            },
          ],
        };
      }
